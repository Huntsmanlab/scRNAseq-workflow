...
plot_markers <- function(paths, ids, ntop = 100, negative_control = FALSE, randomSubsets = 100){
  ...
  # 
  if(length(sce[sce$id == ids[1],]) > length(sce[sce$id == ids[2],])) {
    larger = ids[[1]]
    smaller = ids[[2]]
  } else {
    larger = ids[[2]]
  ...
  degs <- fMarkersSampling(sce, smaller, larger, randomSubsets)
...
paths <- as.list(unlist(strsplit(params$sce_norm, ' '))) 
outputs <- plot_markers(paths, ids = ids, ntop = 50)
colnames(outputs$markers_all)[5] <- "logFC"

# Save DGE data
outDF = outputs$markers_all
saveRDS(outDF, file = params$output_path_paired)

```

```{r fig.height= 10, fig.width= 12}

if(is.null(outputs$splot)){ # if no random sampling:
  outputs$plot
