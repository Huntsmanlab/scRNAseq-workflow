...
plot_markers <- function(paths, ntop = 50, negative_control){

  sce_list <- lapply(paths, function(some_path) readRDS(some_path))
  
  ids <- lapply(sce_list, function(sce) unique(sce$id))
  
  # sce_list <- scran_batch_norm(sce_list)
  sce_list <- intersect_all(sce_list)
  # sce_list <- do.call(batchelor::multiBatchNorm, c(sce_list, assay.type="counts", min.mean = 0.1))
  
  # before combining make sure we have the same row numbers in our sces 
  # intersect_all
 ...
 # remove genes whose total counts < 100 and detected in < 5% of cells, remove mito, ribo genes
  ...
  markers <- markers[[1]] # pos logFC = up-regulated in tranduced cells
...
paths <- as.list(unlist(strsplit(params$sce_clus[[1]], ' '))) 
