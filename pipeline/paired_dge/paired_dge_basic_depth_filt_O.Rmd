...
params:
  ...
  ins: '/huntsman/general/data/clustered/sce/DH35_eGFPMAYA/sce_clus.rds'
...
plot_markers <- function(inputs, ids, ntop = 50, negative_control = FALSE, randomSubsets = 100){
  ...
  if (length(strsplit(inputs, "-")[[1]]) == 1) {
    ...
    if(is.null(rowData(sce)$qc_pass)) {
      sce <- gene_filter(sce, gene_min_counts = 100, gene_min_detection_rate = 5, reporters = "eGFP-MAYA") }
  } else {
  ...
  depth_cutoff <- quantile(sce@colData@listData[["total"]], 0.5) 
  sce <- sce[,which(sce@colData@listData[["total"]] > depth_cutoff)]
...
if (length(clusters) < 3) {
  if(is.null(outputs$splot)){ # if no random sampling:
    ...
    plot_grid(output$splot, outputs$plot, labels = "AUTO")
  }
} else {
  writeLines('Not yet implemented')
}

## If you want to save a nice volcano plot and sent to researchers:
