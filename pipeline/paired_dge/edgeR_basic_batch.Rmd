---
title: 'DGE - Comparing sample2 to sample1'
...
params:
  ids: 'DH3-DH10=DH15-DH16'
  output_path_up: 'output_path_up'
  output_path_down: 'output_path_down'
  seed: 123
---
...
ids_1 <- ids_1_name
ids_1_split <- strsplit(ids_1, "-")[[1]] # split by "-" 

ids_2_name <- strsplit(params$ids, "=")[[1]][[2]] # split by "=" 
ids_2 <-ids_2_name
ids_2_split <- strsplit(ids_2, "-")[[1]] # split by "-" 

```

```{r PSEUDOBULKING, include = FALSE}

# now check how many samples we are given 
# if we have at least two replicates for each sample, follow this path: 

if (length(ids_1_split) + length(ids_2_split) >= 4) {
  
  integrated_pair1 <- readRDS(here('..', 'data', 'batch_corrected', paste(ids_1_name), 'corrected.rds'))
  integrated_pair2 <- readRDS(here('..', 'data', 'batch_corrected', paste(ids_2_name), 'corrected.rds'))

  # make pseudo counts for each sample
  integrated_pair1_df <- make_pseudo_counts_seurat(integrated_pair1)
  integrated_pair2_df <- make_pseudo_counts_seurat(integrated_pair2)
...
} else { # this is what we do if we dont have at least two replicates for each sample 
  
  integrated_pair1 <- readRDS(here('..', 'data', 'clustered', 'sce', paste(ids_1_name), 'sce_clus.rds'))
  integrated_pair2 <- readRDS(here('..', 'data', 'clustered', 'sce', paste(ids_2_name), 'sce_clus.rds'))
