---
title: 'DGE - Comparing strsplit(params$ids, "=")[[1]][[2]] to strsplit(params$ids, "=")[[1]][[1]]'
...
sample1 <- readRDS(path_sample1[1])
sample2 <- readRDS(path_sample2[1])

# some quick batch correction 
sce_list <- scran_batch_norm(list(sample1, sample2))
sample1 <- sce_list[[1]]
sample2 <- sce_list[[2]]

# extract assay data 
sample1_df <- as.data.frame(counts(sample1))
sample2_df <- as.data.frame(counts(sample2))

# find common genes, subset to that and cbind all the samples
combined <- find_and_bind(list(sample1_df, sample2_df))

# now make the group, we will use this to make the DGE object later on 
group <- c(rep(1, ncol(sample1_df)), rep(2, ncol(sample2_df)))
...
pFiltered = data
pFiltered$pvalue[pFiltered$pvalue == 0] = 9.9e-324
x_limits = max(max(upregulated_genes_table[, 2]), abs(min(downregulated_genes_table[, 2])))
y_limit = max(-log10(pFiltered), na.rm=TRUE)


EnhancedVolcano(pFiltered,
  lab = rownames(data),
  x = 'log2FoldChange',
  y = 'pvalue',
  ...
  labCol = 'black',
  labFace = 'bold',
  #drawConnectors = TRUE, # Breaks plotting
  widthConnectors = 0.4,
  colConnectors = 'black',
  pointSize = 0.7, 
  ...
  FCcutoff = 1, 
  pCutoff = 10e-5, 
  legendLabels = c('not significant','Log (base 2) fold-change','P value',
