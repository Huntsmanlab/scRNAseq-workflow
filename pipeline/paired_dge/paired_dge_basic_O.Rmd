...
params:
  ...
  reporter: 'Epithelial cells, Epithelial ciliated cells'
  ...
  output_path: '/huntsman/mdouglas/dh_organoid/data/dge/VOA10819UT_DBZ'
  # INSTRUCTIONS: Name of the output data file
  output_file: 'outputs_27072020.rds'
...
source(here('source_scripts', 'utility_m.R'))
source(here('source_scripts', 'utils_dge.R'))
...
plot_markers <- function(inputs, ids, ntop = 50, negative_control, randomSubsets = 100){
  ...
  if ((params$reporter == FALSE) && (params$clusters == FALSE)) {                                         # Sample 1 vs Sample 2...
    sce$comparator <- sce$id
    clusters <<- unique(sce$id)
    ...
    reportercount <- sce@assays@data@listData[["counts"]][c(params$reporter),]
    ext_data1 <- mutate(as.data.frame(reportercount), class = if_else(reportercount == 0, ids[2], params$reporter))
  ...
  for (i in clusters) {
    ...
    if (params$unequal != FALSE) {
      degs <- fMarkersSampling(sce, clusters[1], clusters[2], randomSubsets)
