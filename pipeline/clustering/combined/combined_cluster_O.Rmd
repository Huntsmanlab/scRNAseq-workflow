...
```{r include = FALSE}
# we will be running similar analyses on two datasets, so we have this function instead: 
# this function converts sce into a seurat object and perform clustering. 
...
frequency table: 
```{r include = FALSE}
# we run this function on our sces here. we just convert them into seurat objects and do a bit of a clean up 
...
# condition based on cell number: 
...
```{r include = FALSE}
# MOVE THIS TO DIM REDUCTION 
# cluster.averages <- AverageExpression(samples.integrated, return.seurat = TRUE)
# 
# head(AverageExpression(samples.integrated))
# 
# 
# DoHeatmap(cluster.averages, features = unlist(TopFeatures(samples.integrated[["pca"]], balanced = TRUE)), size = 3, 
#     draw.lines = FALSE)
# 
# 
# 
# head(cluster.averages[["integrated"]])
```