...
for(i in 1:length(reporters)){
  if(any(grepl(reporters[[i]], rownames(sce), ignore.case = TRUE) == TRUE)){
    ...
    df_umap$reporter <- count_reporter
...
```{r, eval=FALSE}
#Here, we plot heatmap showing logFC for signature genes in each cluster. Columns represent clusters with cluster number at the bottom of the heatmap. Signature genes with positive logFC are upregulated in this cluster compared to others; and negative logFC genes are downregulated.
...
for(i in 1:length(markers.sce)){

  # subsets the interesting marker
  interesting <- markers.sce[[i]]
  interesting[1:20,1:4]
  best.set <- interesting[interesting$Top <= 6,]
  logFCs <- getMarkerEffects(best.set)

  print(pheatmap(logFCs, breaks=seq(-5, 5, length.out=101),
           main = paste0("signature genes for cluster ", i)))
}
...
# get top 10 DEGs from clusters:
genes <- sig_gene_table(markers.sce.up, top=10) %>% unlist(use.names = FALSE) %>% unique()

make_heatmap <- function(sce, all_markers, col_annot){
...
my_colours <- list(cluster_colors, sample_colors)
...
col_annot <- data.frame(colnames(sce), sce$cluster, row.names = colnames(sce))
names(col_annot) <- c("column_names", "cluster")
...
print(make_heatmap(sce, genes, col_annot))
