...
#### In this document, we are working on DH8 (endometrioid_ovarian). In the published paper, it seems like DH8 (or OV1) forms two clusters on the TSNE plots, we will make the TSNE plot again and see if there any differentially expressed genes between the two clusters. 
...
Blue symbolizes downpregulated genes in cluster2 compared to cluster1.  
...
The same gene table, after we eliminate genes with logfold changes less than 1.   
```{r echo = FALSE, message = FALSE}
...
These are the downregulated GO pathways in cluster2 compared to cluster1. Therefore, they would be upregulated in cluster2 compared to cluster1. 
```{r echo = FALSE, message = FALSE}
# here we show the results of the go pathways 
...
#### KEGG analysis  
Upregulated in cluster2 compared to cluster1  
```{r echo = FALSE, message = FALSE}
...
Downregulated in cluster2 compared to cluster1  
```{r echo = FALSE, message = FALSE}
