...
parser <- ArgumentParser(description = "Make sce")
...
parser$add_argument('--output_file_name', metavar='FILE', type='character', help="Path to sce")
...
if(dir.exists(paste(args$path_to_10X, 'filtered_feature_bc_matrix', sep = '/')) == TRUE) {
    
    path_to_10X <- paste(args$path_to_10X, 'filtered_feature_bc_matrix', sep = '/')
    sce <- read10xCounts(samples = path_to_10X) 
    
} else {sce <- read10xCounts(samples = args$path_to_10X) }
...
saveRDS(sce, file = args$output_file_name) #save the sce object we made in this function 
