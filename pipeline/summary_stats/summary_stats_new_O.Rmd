...
params:
  pair_ids: 'DH1_DBZ-DH1_control' # to run this, write TWO ids here, separated by -
...
PlotDiffMap <- function(dat, dm_path, dim){
  ...
  if(!file.exists(dm_path)){
    ...
    dir.create(dirname(dm_path)) # create sample folder
...
PlotReporter <- function(reporters, sce){
  ...
  p2[[i]] <- plot_ly(data = df_umap,
    ...
    p[[i]] <- subplot(p2[[i]], p3[[i]]) %>% layout(title=dh_organoid_ids[[i]])
  }
  }
  return(p)
}
