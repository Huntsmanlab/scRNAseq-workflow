...
params:
  pair_ids: 'DH35' # to run this, write TWO ids here, separated by -
  ...
  ribo_thresh_max: 60 
  nmads: 3
  remove_mito_and_ribo: "no" # "yes" will remove all mito and ribo genes, "no" will retain them both in the sce
  dm_path: dm_path
...
for(i in 1:length(dh_organoid_ids)){
  ...
  sce_qcs[[i]] <- make_sce_qc(params$whichMethod, sce_raws[[i]], params$mito_thresh_max, 
                              params$mito_thresh_min, params$ribo_thresh_max, 
...
# Initialize seurat object in preparation of the merging
...
dim_red_plot <- function(seurat_obj){
  ...
  seurat_obj <- RunTSNE(seurat_obj, dims = 1:30, check_duplicates = FALSE, perplexity = 10) #change perplexity to a smaller number for samples with few cells
...
process_seurat <- function(seurat_obj){
  ...
  clustered <- ScaleData(normalized) %>% 
    ...
    RunTSNE(dims = 1:30, dim.embed = 3, check_duplicates = FALSE) %>% # change perplexity to a smaller number for samples with few cells
    FindNeighbors(dims = 1:10) %>% 
    FindClusters(resolution = 0.5) # larger resolution gives more clusters; recommended resoluation range is 0.4-1.2 for dataset with ~3k cells
...
dm_path <- lapply(dh_organoid_ids, function(id) here('..','..','general','data','dim_reduction',id,'dm.rds'))
...
for(i in 1:length(pro_sample)){
  ...
  for(j in 1:length(ind)){
    ...
    deg <- temp$gene
    if(length(temp$gene) < 10) deg <- c(temp$gene, rep(NA, 10-length(temp$gene)))
    df[,j] <- deg
}
