...
source('//huntsman/amunzur/DH_organoid/pipeline/summary_stats/qcPlots.R')
...
makeTable <- function(df, index) {
  ...
  df <- as.data.frame(df) %>% 
    ...
    select(id, cell_number, sum, detected, subsets_mito_sum, subsets_mito_percent) %>% 
    
    # and here we just rename them for clarity 
    rename(RNA_reds = sum, detected_genes = detected, mito_reads = subsets_mito_sum, mito_percent = subsets_mito_percent)
