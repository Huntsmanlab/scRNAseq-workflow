...
# and here is a while loop to sum across the clusters for each cluster in the list 
i <-  1 
...
Clusters far apart mean they that are different in terms of gene expression. Closer means more similar. 
...
Biological coefficient of variation is the true variation between genes. 
...
In this script, we squeeze the dispersion values from raw counts towards the trended dispersion. (This just means we transform the data in certain ways.)
The plots gives an idea about how much the data was squeezed.  
...
The blue lines indicate 2-fold up or down. The number of genes that are significantly up or down regulated are shown in the table, according to the plot.  
...
# formattable(upregulated_genes_table, align = c("l", rep("r", NCOL(upregulated_genes_table) - 1)))

datatable(upregulated_genes_table)

# upregulated_genes_table %>% 
#     knitr::kable(caption = paste('comparing', dh_organoid_ids[[2]], 'to', dh_organoid_ids[[1]])) %>% 
#     kable_styling(full_width = F)
...
# 
# gotable %>% 
#   knitr::kable(caption = 'GO pathways') %>% 
#   kable_styling(full_width = F)
...
# keggtable %>% 
#   knitr::kable(caption = 'KEGG pathways') %>% 
#   kable_styling(full_width = F)
#   
```
