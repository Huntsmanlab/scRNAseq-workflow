...
pseudoCountsList <- make_pseudo_counts(sces)
...
# shuffle the cluster each cell belongs to for each sce 
sces_1 <- sces[[1]]
sces_2 <- sces[[2]]

sces_1$cluster <- sample(sces_1$cluster)
sces_2$cluster <- sample(sces_2$cluster)

# update the list 
sces <- list(sces_1, sces_2)
...
# now we combine them into one dge object: 
dgeObject_neg <- do.call(cbind, dge_object_list) 
...
fit_neg_control <- glmQLFit(dge_neg_control, designMatrix, robust = TRUE)
