...
# load sce qc
sce_norm <- readRDS(here('..', 'data', 'normalized', params$id, 'sce_norm.rds'))
...
p1 <- plotReducedDim(sce_norm, dimred = 'TSNE', colour_by = 'cluster') + labs(title = paste(params$id, 'Clustering - TSNE', sep = '-'))
...
sce_norm_cas <- readRDS(here('..', 'data', 'cellassign', params$id, 'sce_norm_cas.rds'))
...
p2 <- DimPlot(object = seur_sce, 
  ...
  labs(title = paste(params$id, 'Cell Assign for scRNAseq Cells', sep = '-'))
