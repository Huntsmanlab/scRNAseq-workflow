...
MesenchymalStem = c("ITGB1", "CD44", "NT5E", "THY1", "ENG", "ALCAM", "VCAM1")
...
# cell types we want to include - this is a subset of marker_genes list
cell_types <- c("lymphocyte", "endothelial", "epithelial_ciliated", "epithelial_unciliated","macrophage_cells", "stromal_fibroblast", "vasular_smooth_muscle", "EndometrialStem", "Ovarian_cancer", "mesenchymal")
...
PlotReporter <- function(reporters, seurat_obj, col){
  ...
  # get genes in common
  reporters <- intersect(rownames(seurat_obj[[1]]), reporters)
  ...
  for(i in 1:length(reporters)){
      for(j in 1:length(seurat_obj)){
        ...
        # get umap and tsne coordinates for plotting
        df_umap <- cbind(seurat_obj[[j]][["TSNE"]]@cell.embeddings %>% as.data.frame(), 
                         seurat_obj[[j]][["UMAP"]]@cell.embeddings %>% as.data.frame())
      ...
      p[[i]][[j]] <- plot_ly(data = df_umap,
        ...
        mode = "markers",
        marker = list(size = 4), 
        colors = brewer.pal(6, col[j])) %>% 
          layout(title=paste0(reporters[[i]], "_logcount"),
                 legend= list(itemsizing='constant'), xaxis = ax, yaxis = ax, showlegend=F)
      }
    } 
    print(do.call("subplot", c(p[[i]])))
  }
}


PlotReporter(reporters=epithelial_unciliated, seurat_obj=expr_obj, col=c("Reds", "Blues"))
...
PlotReporter <- function(reporters, seurat_obj, col){
  ...
  # initialize a list that can be printed out in an html report
  p <- replicate(length(seurat_obj), vector("list"), simplify = FALSE)
  ...
  ax <- list(zeroline = FALSE, showline = TRUE, showgrid = FALSE)
      ...
      for(j in 1:length(seurat_obj)){
        ...
        if(any(grepl(paste("^",reporters,"$", sep=""), rownames(seurat_obj[[j]]), ignore.case = TRUE) == TRUE)){
        # if our sample contains one of the reporter genes: get the logcounts of this gene
        reporter_idx <- grep(paste("^",reporters,"$", sep=""), rownames(seurat_obj[[j]]), ignore.case = TRUE)
        # add logcount 
        count_reporter <- count[reporter_idx,]
        df_umap$logcounts <- count_reporter
  
        # plot
      p[[j]] <- plot_ly(data = df_umap,
        x = ~TSNE_1, y = ~TSNE_2,
        opacity = 1,
        color = ~logcounts,
        type = "scatter",
        ...
        marker = list(size = 4), 
        colors = brewer.pal(6, col[j])) %>% 
          layout(title=paste0(reporters, "_logcount"),
                 legend= list(itemsizing='constant'), xaxis = ax, yaxis = ax, showlegend=F)
        }
        #do.call("subplot", c(p[[j]])) this doesn't work for some reason
      } 
  subplot(p[[1]],p[[2]])
  }
...
# get genes in common
reporters <- intersect(rownames(expr_obj[[1]]), MesenchymalStem)

hm <- htmltools::tagList()
for(i in 1:length(reporters)){
  hm[[i]] <- PlotReporter(reporters=reporters[i], seurat_obj=expr_obj, col=c("Reds", "Blues"))
}
hm
