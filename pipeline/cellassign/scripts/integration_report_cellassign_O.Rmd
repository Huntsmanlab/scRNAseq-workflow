...
params:
  pair_ids: 'DH21_NEW-DH21_control' # write here the ids you want to integrate, separate by '-'
...
sample_type <- "other" # if granulosa samples, change to "GC"; if .. samples, change to ..
sce_clus <- lapply(id.list, function(id) here('..', 'data', 'clustered', 'sce', id, 'sce_clus.rds'))
cell_type_csv <- lapply(id.list, function(id) here('..', 'data', 'cellassign', id, 'cell_types.csv'))
...
for(i in 1:length(id.list)){
  if(file.exists(cell_type_csv[[i]])==TRUE){
    ...
    # run cell assign and get sce with cell type information
    sce_cas[[i]] <- run_cellassign(sce_clus[[i]], sample_type, cell_type_csv[[i]])
  }
}

# load scran batch corrected objects
scran <- readRDS(here('..', 'data', 'batch_corrected', params$pair_ids, 'scran_corrected.rds'))

# load seurat batch corrected objects
integrated <- readRDS(here('..', 'data', 'batch_corrected', params$pair_ids, 'seurat_corrected.rds'))
