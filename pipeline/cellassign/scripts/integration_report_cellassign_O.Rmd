...
params:
  pair_ids: 'DH31' # write here the ids you want to integrate, separate by '-'
...
sce_clus <- lapply(id.list, function(id) here('..', '..', 'general', 'data', 'clustered', 'sce', id, 'sce_clus.rds'))
cell_type_csv <- lapply(id.list, function(id) here('..', '..', 'general', 'data', 'cellassign', id, 'cell_types.csv'))
cell_types <- c("epithelial_secretory", "epithelial_ciliated", "endothelial", "stromal_fibroblast", "vasular_smooth_muscle", "lymphocyte", "mesenchymal", "mesenchymal_stem", "endometrial_stem", "ovarian_cancer")
...
if(length(id.list) > 1){
  ...
  integrated <- readRDS(here('..', '..', 'general', 'data', 'batch_corrected', params$pair_ids, 'seurat_corrected.rds'))

  # load scran object
  scran <- readRDS(here('..', '..', 'general', 'data', 'batch_corrected', params$pair_ids, 'scran_corrected.rds'))
...
det_cell_type <- rownames(marker_mat)
...
Below are tsne and umap plots showing clustering and cell assignment results for sample integrated and corrected with seurat.
...
# # append the id name in front of the barcodes; subset sce and integrated object to common barcodes
# celltype <- list()
# for (i in 1:length(sce_cas)) {
#   colnames(sce_cas[[i]]) <- paste(id.list[[i]], colnames(sce_cas[[i]]), sep = '.')
#   common_names <- intersect(colnames(integrated), colnames(sce_cas[[i]]))
#   celltype[[i]] <- colData(sce_cas[[i]])$cell_type[colnames(sce_cas[[i]]) %in% common_names]
# }
# 
# # add cell types to the integrated object
# integrated@meta.data$cell_type <- unlist(celltype)
