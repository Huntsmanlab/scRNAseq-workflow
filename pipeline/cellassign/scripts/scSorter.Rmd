...
marker_genes <- data.frame(Type=c(rep("epithelial_secretory", length(epithelial_secretory)),
                           ...
                           Marker=c(epithelial_secretory, epithelial_ciliated, endothelial, stromal_fibroblast, 
                                    lymphocyte, macrophage_cells, vasular_smooth_muscle, MesenchymalStem, 
                                    Ovarian_cancer, EndometrialStem, Mesenchymal_cells),
                           stringsAsFactors = F)
...
if(file.exists(cell_type_csv)==TRUE){ # if cell_type_csv already exists
  cell_type <- read_csv(cell_type_csv)
    ...
    sce_cas <- run_cellassign(sce, cell_types, cell_type_csv)
