...
sce <- as.list(unlist(strsplit(params$sce_clus[[1]], ' '))) 
sce <- lapply(sce, function(path) readRDS(path)) 
...
We have expression level (logcounts) for `r id.list[[1]]` on the left and `r id.list[[2]]` on the right. The cell type we want to take a closer look at is `r params$marker`.
...
marker <- marker_genes %>% dplyr::filter(Type == marker) %>% dplyr::select(Marker) %>% unlist(use.names = F)

# get genes in common
reporters <- intersect(rownames(expr_obj[[1]]), marker)
