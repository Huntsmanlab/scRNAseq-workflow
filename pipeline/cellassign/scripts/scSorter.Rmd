...
cell_types <- c("lymphocyte", "endothelial", "macrophage_cells", "stromal_fibroblast", "MesenchymalStem", "vasular_smooth_muscle", "EndometrialStem", "Ovarian_cancer", "Mesenchymal_cells", "epithelial_unciliated", "epithelial_ciliated")
...
PlotReporter <- function(reporters, seurat_obj, col, id){
  ...
  for(i in 1:length(reporters)){
    ...
    df_umap <- cbind(seurat_obj[["TSNE"]]@cell.embeddings %>% as.data.frame(), seurat_obj[["UMAP"]]@cell.embeddings %>% as.data.frame())
    ...
    p3[[i]] <- plot_ly(data = df_umap,
        ...
        colors = brewer.pal(6, col)) %>% 
          layout(title=paste0(id, ": ",reporters[[i]], "_logcount"),
...
for(i in 1:length(expr_obj)){
  PlotReporter(reporters=epithelial_ciliated, seurat_obj=expr_obj[[i]], col="Reds", id=id.list[[i]])
}
